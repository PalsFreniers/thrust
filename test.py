...
def run_test_for_file(file_path: str, stats: RunStats = RunStats()):
    ...
    if tc is not None:
        ...
        com = cmd_run_echoed(["pypy", "thrust.py", "-c", "run", "clean", file_path, *tc.argv], input=tc.stdin, capture_output=True)
        ...
        com = cmd_run_echoed(["pypy", "thrust.py", "-c", "clean", file_path])
        if com.returncode != 0:
            error = True
            stats.failed += 1
        stats.ignored += 1
...
def update_output_for_file(file_path: str):
    ...
    output = cmd_run_echoed(["pypy", "thrust.py", "-c", "clean", "run", file_path, *tc.argv], input=tc.stdin, capture_output=True)
...
def usage(exe_name: str):
    ...
    print("  SUBCOMMAND:")
    print("    run [TARGET]")
    print("      Run the test on the [TARGET]. The [TARGET] is either a *.thrust file or ")
    print("      folder with *.thrust files. The default [TARGET] is './examples/'.")
    print()
    ...
    print()
    print("        output [TARGET]")
    print("          Update the output of the [TARGET]. The [TARGET] is either a *.thrust")
    print("          file or folder with *.thrust files. The default [TARGET] is")
    print("          './examples/'")
    print()
...
if __name__ == '__main__':
    ...
    if subcommand == 'update' or subcommand == 'record':
        ...
        cmd_run_echoed(['mypy', './thrust.py'])
        run_test_for_folder('euler')
        run_test_for_folder('examples')
        run_test_for_folder('challenges')
