...
def run_test_for_file(file_path: str, stats: RunStats = RunStats()):
    ...
    if tc is not None:
        ...
        com = cmd_run_echoed(["pypy", "thrust.py", "-c", "run", file_path, *tc.argv], input=tc.stdin, capture_output=True)
        ...
        com = cmd_run_echoed(["pypy", "thrust.py", "-c", file_path])
        if com.returncode != 0:
            error = True
            stats.failed += 1
        stats.ignored += 1
...
def update_output_for_file(file_path: str):
    ...
    output = cmd_run_echoed(["pypy", "thrust.py", "-c", "run", file_path, *tc.argv], input=tc.stdin, capture_output=True)
