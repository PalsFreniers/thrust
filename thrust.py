...
def type_check_program(program: Program, contracts: Dict[str, Contract], extern, entry):
    visited_dos: Dict[OpAddr, DataStack] = {}
    contexts: List[Context] = []
    if len(program.ops) == 0:
        exit(0);
    if not Library and contracts[entry][0].ins != [] and contracts[entry][0].outs != [DataType.INT]:
        compiler_error(Loc(entry, 0, 0), "function %s should have signature `fn %s -> int as`" % (entry, entry))
        exit(1)
    if not Library and ("init" in contracts):
        global GlobalInit
        GlobalInit = True
        if contracts["init"][0].ins != [] and contracts["init"][0].outs != []:
            compiler_error(Loc(entry, 0, 0), "function init should have signature `fn init as`" % (entry, entry))
            exit(1)
    ...
    while len(contexts) > 0:
        ...
        if op.typ == OpType.PUSH_BOOL:
            ...
            assert len(Intrinsic) == 47, "Exhaustive intrinsic handling in type_check_program()"
...
def generate_nasm_linux_x86_64(program: Program, out_file_path: str, extern: List[str], start: str, entry: str):
    strs: List[bytes] = []
    with open(out_file_path, "w") as out:
        ...
        for ip in range(len(program.ops)):
            ...
            if op.typ == OpType.PUSH_INT:
                ...
                assert len(Intrinsic) == 47, "Exhaustive intrinsic handling in generate_nasm_linux_x86_64()"
                if op.operand == Intrinsic.MIN:
                    out.write("    ;; -- min --\n")
                    out.write("    pop rax\n")
                    out.write("    pop rbx\n")
...
assert len(Keyword) == 24, "Exhaustive KEYWORD_NAMES definition."
KEYWORD_NAMES = {
    'if': Keyword.IF,
    'elif': Keyword.ELIF,
    'else': Keyword.ELSE,
...
assert len(Intrinsic) == 47, "Exhaustive INTRINSIC_BY_NAMES definition"
INTRINSIC_BY_NAMES = {
    '+': Intrinsic.PLUS,
    '-': Intrinsic.MINUS,
    '*': Intrinsic.MUL,
...
def check_valid_symbol(token: Token, name: str, macros: Dict[str, Macro], functions: Dict[str, Function], memories: Dict[str, Memory], consts: Dict[str, Const], structs, aliases):
    if name in macros:
    ...
    if name in structs:
        ...
        exit(1)
    if name in aliases:
        compiler_error_with_expansion_stack(token, "redefinition of already existing alias `%s`" % name)
        compiler_note(consts[name].loc, "the first definition is located here")
        exit(1)
...
def eval_expression(rtokens: List[Token], macros: Dict[str, Macro], consts: Dict[str, Const], funcs: Dict[str, Function], structs, iota: List[int]) -> Tuple[int, DataType]:
    ...
    while len(rtokens) > 0:
        ...
        if token.typ == TokenType.KEYWORD:
            if token.value == Keyword.END:
                break
            elif token.value == Keyword.SIZEOF:
                if len(rtokens) == 0:
                    compiler_error_with_expansion_stack(token, "expected type name but found nothing")
                    exit(1)
                typ = rtokens.pop()
                if(typ.typ != TokenType.WORD):
                    compiler_error_with_expansion_stack(token, "expected type name to be %s but got %s" % (human(TokenType.WORD), human(token.typ)))
                    exit(1)
                if typ.value in DATATYPES:
                        stack.append((getTypeSize(typ.value), DataType.INT))
                if typ.value in structs:
                    tmpStruct = structs[typ.value]
                    ac = 0
                    for ttyp in tmpStruct.types:
                        ac += getTypeSize(ttyp)
                    stack.append((ac, DataType.INT))
...
def parse_function_signature(rtokens: List[Token], structs: Dict[str, Struct], aliases: Dict[str, DataType], last: Keyword) -> Contract:
    ...
    while len(rtokens) > 0:
        ...
        if tok.typ == TokenType.WORD:
            ...
            elif tok.value in aliases:
                if isIn:
                    signature.ins.append(aliases[tok.value])
                else:
                    signature.outs.append(aliases[tok.value])
            else:
...
def getTypeSize(typ: DataType) -> int:
...
def parse_program_from_tokens(tokens: List[Token], include_paths: List[str], expansion_limit: int) -> Program:
    ...
    while len(rtokens) > 0:
        ...
        if token.typ == TokenType.WORD:
            ...
            assert len(Keyword) == 24, "Exhaustive keywords handling in parse_program_from_tokens()"
            if token.value == Keyword.TRUE:
                ...
                if cur_proc is not None:
                    compiler_error_with_expansion_stack(token, "scoped includes not allowed")
                ...
                if cur_proc is not None:
                    compiler_error_with_expansion_stack(token, "scoped structures not allowed")
                    compiler_note(cur_proc.loc, "current procedure stating here")
                    exit(1)
                if len(rtokens) == 0:
                ...
                if token.typ != TokenType.WORD:
                    ...
                    exit(1)
                assert isinstance(token.value, str), "This is probably a bug in the lexer"
                check_valid_symbol(token, token.value, macros, functions, memories, consts, structs, aliases)
                ...
                if cur_proc is None:
                    ...
                    check_valid_symbol(token, token.value, macros, functions, cur_proc.memories, consts, structs, aliases)
                    cur_proc.memories[memname] = Memory(memloc, cur_proc.memCapacity)
                    cur_proc.memCapacity += memsize
            elif token.value == Keyword.CONST:
                if cur_proc is not None:
                    compiler_error_with_expansion_stack(token, "scoped constants not allowed")
                    compiler_note(cur_proc.loc, "current procedure stating here")
                    exit(1)
                if len(rtokens) == 0:
                ...
                if cur_proc is not None:
                    compiler_error_with_expansion_stack(token, "scoped macros not allowed")
                    compiler_note(cur_proc.loc, "current procedure stating here")
                    exit(1)
                if len(rtokens) == 0:
                ...
                ip += 1
                check_valid_symbol(token, token.value, macros, functions, memories, consts, structs, aliases)
                signature = parse_function_signature(rtokens, structs, aliases, Keyword.AS)
                proc_loc = token.loc
                ...
                if(typ.typ != TokenType.WORD):
                    ...
                    exit(1)
                if typ.value in DATATYPES:
                    program.ops.append(Op(typ=OpType.PUSH_INT, operand=getTypeSize(DATATYPES[typ.value]), token=typ))
                if typ.value in structs:
                    tmpStruct = structs[typ.value]
                    ac = 0
                    for ttyp in tmpStruct.types:
                        ac += getTypeSize(ttyp)
                    program.ops.append(Op(typ=OpType.PUSH_INT, operand=ac, token=typ))
                ip += 1
                ...
                for c in token.value:
                    if not (c.isalnum() or c == '_'):
                        ...
                        exit(1)
                check_valid_symbol(token, token.value, macros, functions, memories, consts, structs, aliases)
                signature = parse_function_signature(rtokens, structs, aliases, Keyword.END)
                proc_loc = token.loc
                externs[token.value] = Function(token.loc, 0, {}, 0, signature, token.value)
            elif token.value == Keyword.ASSERT:
                if len(rtokens) == 0:
                    compiler_error_with_expansion_stack(token, "expected expression but found nothing")
                    exit(1)
                ...
                typ = None
                if tok.value in DATATYPES:
                    typ = DATATYPES[tok.value]
                elif tok.value in aliases:
                    typ = aliases[tok.value]
                else:
                    compiler_error_with_expansion_stack(tok, "alias name should be a type or another alias" % (human(TokenType.WORD), human(token.typ)))
                    exit(1)
                if len(rtokens) == 0:
                    compiler_error_with_expansion_stack(tok, "expected `as` keyword but found nothing")
                    exit(1)
                tok = rtokens.pop()
                if tok.typ != TokenType.KEYWORD:
                    compiler_error_with_expansion_stack(token, "expected keyword `as`")
                    exit(1)
                if tok.value != Keyword.AS:
                    compiler_error_with_expansion_stack(token, f"expected keyword `as` but founded {tok.text}")
                    exit(1)
                if len(rtokens) == 0:
                ...
                tok = rtokens.pop()
                if tok.typ != TokenType.WORD:
                    compiler_error_with_expansion_stack(token, "expected alias name to be %s but got %s" % (human(TokenType.WORD), human(token.typ)))
                    exit(1)
                check_valid_symbol(token, token.value, macros, functions, memories, consts, structs, aliases)
                aliases[tok.value] = typ
                ...
                compiler_error_with_expansion_stack(token, f"keyword `{token.text}` is supported only in function signatures and aliases")
                exit(1)
            else:
                print(token)
                assert False, 'unreachable';
...
if __name__ == '__main__' and '__file__' in globals():
    ...
    if subcommand == "-c":
        ...
        if not unsafe:
            type_check_program(program, contracts, externs, entry)
        if not silent:
            print("[INFO] Generating %s" % (basepath + ".asm"))
        generate_nasm_linux_x86_64(program, basepath + ".asm", [names for names in externs.keys()], start, entry)
        cmd_call_echoed(["nasm", "-felf64", basepath + ".asm"], silent)
        #cmd_call_echoed(["rm", '-fr', basepath + ".asm"], silent)
        if not Library:
        ...
        cmd_call_echoed(["rm", '-fr', basepath + ".o"], silent)
